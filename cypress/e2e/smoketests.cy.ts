...
describe("Test pages", () => {
  it("Basic MDX Rendering", () => {
    ...
    cy.findByRole("heading", { name: "This is a basic MDX test." }).should(
      "exist"
    );
  ...
  it("Image rendering", () => {
    ...
    cy.findByRole("img", { name: "this is the image alt text" }).should(
      "exist"
    );
  });
  ...
  it("Frontmatter ", () => {
    ...
    cy.get('meta[name="twitter:title"][content="I am the title"]').should(
      "exist"
    );

    cy.get('meta[name="description"][content="I am the description"]').should(
      "exist"
    );
    cy.get(
      'meta[name="twitter:description"][content="I am the description"]'
    ).should("exist");
  });
  ...
  it("series - has the right content", () => {
    ...
    cy.findByRole("link", { name: "Next: Series - Post 2" }).click({
      force: true,
    });
    cy.findByText("I am series - post 2 content").should("exist");
    cy.findByRole("link", { name: "Next: Series - Post 2" }).should(
      "not.exist"
    );
  });
  ...
  it("/test will show a test index page", () => {
    ...
    cy.request({ url: "/test", failOnStatusCode: false })
      .its("status")
      .should("be.ok");
  });
