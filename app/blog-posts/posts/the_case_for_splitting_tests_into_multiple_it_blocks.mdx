---
meta:
  twitter:title: The case for splitting Cypress tests into multiple it blocks
  twitter:description: Cypress recommends long running tests with lots of assertions. I argue that it would be helpful if we had granular visiblity of the parts of the test.
  title: The case for splitting Cypress tests into multiple it blocks
  description: Cypress recommends long running tests with lots of assertions. I argue that it would be helpful if we had granular visiblity of the parts of the test.
...
- "Given that I am on the widget page, I click the 'add wiget' button, fill the form, hit submit, and see the new widget"
...
1. It's more work. Now, in order to write some tests, we need to understand what API calls are made, provide mock data, and also possibly be defining some implementation behaviour (for example, "when I call DELETE /widgets/123, the widget with id 123 will no longer be returned in the GET /widgets" call). We're essentially reimplmenting our API in order to test. 
...
And we `itSerial.only` that delete widget test, then all preceeding itSerials in the describe will also run. Performance-wise this is no different to the 'one big test' approach. 
