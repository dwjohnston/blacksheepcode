---
meta:
  twitter:title: Thoughts about JSON API
  twitter:description: JSON API is an opinionated REST API structuring spec. Here's my thoughts.
  title: Thoughts about JSON API
  description: JSON API is an opinionated REST API structuring spec. Here's my thoughts.
...
- Filter lists of resources - again if it's not feasable to return every instance of a resource, we also need our API to allow us to filter resources for us. 
...
For the most part, we can pass these TodoItems around and everything is hunkydory. But where this commonly gets painful is in doing CRUD operations - where creating a Todo we don't currently have an ID, and for patching a Todo we don't need all of the fields. 
...
So I think it's reasonable to have standard wrapper object for resources, that contains the metadata as a seperate property. 
...
I would suggest that _all_ endpoints that return a list, return responses of this shape, even if there are only ever a handful of items. This makes for a consistent developer experience. (You can just ignore any pagination query paramaters, and return a pageSize of 'all'). 
...
Crucially I suggest not doing an 'expansion' type stategy, where the for example TodoItem would look like: 
...
JSON API's opinion is to such attributes in seperate area of the response: 
