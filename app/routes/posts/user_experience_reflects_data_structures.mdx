...
> Any designed user expererience will reflect the underlying data structures
...
Or if we wanted, we could split the flow into two pages, having the user select their favourite genres after they've entered their details, but this just being as frontend facade, we don't actually submit the data until they've finished selecting the genres. 

However, this approach does have a little more friction from a development perspective, there's that little bit of extra logic required to split the form accross two pages and then join the data again for form submission. 
...
<<<<<<< HEAD
We could fudge it, making it a single page, but hiding two API calls in the one form submission. But then we have issues like, 'what if the first API call succeed, but the second one failed'?
=======
...
>>>>>>> 6930b76 (Add User Experience follows data model post)
