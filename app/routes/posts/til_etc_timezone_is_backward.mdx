---
meta:
  twitter:title: "Today I learned: The Etc/GMT+x timezones actually mean UTC-x"  
  twitter:description: Well that's counter intuitive
  title: "Today I learned: The Etc/GMT+x timezones actually mean UTC-x" 
  description: Well that's counter intuitive
...
There are multiple ways to represent timezones - we can talk in terms of their GMT/UTC offset, which is helpful as developers, but as humans we often want to talk in location 'What ever the time is in New York'. 
...
Using tz timezones that explicitly state the offset is handy, because it removes the cognitive burden of having to look up 'Oh yeah America/New_York is UTC-5 or UTC-4 depending on daylight savings time, which is...'.
...
It does appear that POSIX configurations are indeed reversed. For example here's [some Oracle documentation] <sup>[Permalink](https://web.archive.org/web/20230821022910/https://docs.oracle.com/cd/E80921_01/html/esbc_ecz740_configuration/GUID-BDB20D7D-4656-44DF-B823-F7CE52E5CC8A.htm#Configuring-Timezone-using-POSIX-Format)</sup> that states:
