---
meta:
  twitter:title: Black box contract testing - third party API mocking using Mockbin
  twitter:description: We have an API that makes calls out to a third party - how do we test it? 
  title: Black box contract testing - third party API mocking using Mockbin
  description: We have an API that makes calls out to a third party - how do we test it? 
...
3. Click 'disable cache' ‼️ this is an important step. Otherwise you will get a cached result ([304](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/304)), and client behaviour is to not stream that data.
4. Refresh page 
...
The above HAR file doesn't work with Mockbin. It runs into some kind of decompression issue, [I've raised a Github issue on Mockbin here](https://github.com/Kong/mockbin/issues/128). 
...
Possibly removing the headers is going to be problematic for your solution - we might be identify which specific header is causing the trouble. 
...
### Step 2 - Start mockbin and configure (programatic) 

So now we're going to follow these steps, but make these programatic steps that happen as part of running our tests. 
...
I'm not going to paste the code here. The code snippets are well commented, and I'd basically just be copy pasting them here. (As an aside, I would absoultely love some kind of library or plugin that creates code snippets from Github permalinks, just it does if you paste them as a Github comment, for example [see here](https://github.com/dwjohnston/blacksheepcode/issues/40#issuecomment-1590455651).)

Brilliant!

### What I like and don't like about this 

I like that this runs, technically not a single process, but in a single terminal, and cleans up after it self. That is, I don't have to open seperate terminal windows, or run seperate commands before running my tests. 
