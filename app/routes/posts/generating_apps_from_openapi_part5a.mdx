---
meta:
  twitter:title: OpenAPI and contract tests with Jest
  twitter:description: Using good ol' Jest to write some contract tests
  title: OpenAPI and contract tests with Jest
  description: Using good ol' Jest to write some contract tests
...
2. We used the openapi-generator to generate client code. This might be uneccesary, but hey, if it's helping us. 
...
2. In a seperate process run the tests with `yarn test`. 
...
For this, instead of starting our server manually in a seperate terminal window, we'll start the server before each test case, and kill at the end of each test case. 
...
I don't like to retrieve the body as a seperate await call - although that does line up with how HTTP requests work, so I guess I shouldn't complain. 
