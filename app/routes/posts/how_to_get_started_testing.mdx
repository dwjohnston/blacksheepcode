...
A key problem you will need to address in your e2e tests is your philosophy about how data from previous test runs affects the current test run, or how to create the data the this test requires to run. Let's assume our application is a todo list.
...
This does mean that our tests will repeat the same surface area, we will likely have to have multiple tests all doing that same 'create todo' user flow.


##### The 'clean slate' data contains different data you want to test for. 
...
For example, we write a test that does something like "Click create todo. Write 'hello world' in the description box'. Click 'submit'. Expect to see a todo with the name 'hello world'". 

The problem is, if there is a todo named 'hello world' that exists from a previous test run, then your test could actually be failing, but erroneously pass due to the data from the previous test run, or a different test within the same test run.

Another issue you can have is that on paginated lists, your target data may exist later pages, so the test would either fails, or needs to some page traversal logic, which will be time consuming and possibly brittle. 
...
You can read about some of [the difficulties and nuances here](../testing_strategies). 
