---
meta:
  twitter:title: Postman and OpenAPI - Automated Tests
  twitter:description: Postman has an automated testing feature, but how well does it work?
...
2. Postman AFAICT doesn't have a mechanism for reusing queries, say I wanted to compose a collection that executes queries in a certain order (for example, GET then POST then GET), the only way to achieve would be to copy post the queries. 

3. Generally I think where a lot of people get a lot use from Postman, is for making adhoc queries, and so it's always good to still have a tool to do that. Any otherway can be a bit fiddly.
