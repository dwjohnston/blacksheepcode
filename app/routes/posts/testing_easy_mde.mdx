...
<tbody>
    ...
    <tr>
        ...
        <td>Viewing an existing item, and then clicking 'edit' and expecting to see the markdown editor prepopulated.</td>
    </tr>
    <tr>
...
☝️nb. Note for React Testing Library we also need to add some mock configuration to add a `getBoundingClientRect` function to the document. 
...
But this doesn't actually work - the textarea that cypress finds won't actually have the value. 
```js
...
We also need to check that if we submit the form immediately without making any changes, that the intial value will be submitted: 
...
This does not. The problem is, what we can see in the text editor shows the combination of both the intial text and the new entered text, but the textarea element itself only has the newly entered text. 
...
if (formElement) {
    // As promised - the fix for intial value + new data on form submission. 
...
I won't walk through it step by step. 
