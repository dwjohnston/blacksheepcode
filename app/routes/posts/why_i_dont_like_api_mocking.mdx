...
It seems that API mocking, usually using Mock Service Workers (msw) is the default approach for frontend testing these days. 
...
Code for these examples can be found [here](https://github.com/dwjohnston/query-play/commit/e7ccd094ff18658aff192ee9fb141d61b587eeb5). 
...
And our use of `useQuery` is basically the same, we just use function instead of the old `fetchTodos`. 
...
The problem with this, in order to write this test, understand that joining logic. 

In order to read the test, and understand which of the todos will actually be displayed, I similarly need to go in and understand that joining logic. 

As a developer, I don't want to be concerned with this! I'm likely more concerned with what text appears on the screen, the placement of a button etc, and I want to say 'here's my todos, how does it look'. 
...
However, the problem with this apporach is that it doesn't take advantage of react-queries caching mechanism; if we had already loaded all of our todos, labels, and user preferences code would still refetch all of them. 
...
However, this doesn't actually solve the problem, it just shifts it - as likely you are using one of those container components somewhere else in the application, and then, how do you test that component? 
...
It's possible that this 'user preferred todos' feature is an experiemental feature we're building a proof of concept for, and we're building a quick non-optimal to prove the concept, before building a first class backend solution.
 ...
 This actually brings up a second point - it should easy for us to switch out our data retreival logic, potentially, even allow for A/B testing it or feature flagging it. 
