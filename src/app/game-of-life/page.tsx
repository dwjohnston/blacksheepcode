...
function getLivingNeighboursCount(
  ...
  let count = 0;
  for (const yi of yIndexsToUse) {
    for (const xi of xIndexesToUse) {
...
export const GameOfLife = (props: GameOfLifeProps) => {
  ...
  return (
    <div className="game-of-life">
      ...
      <p>
        Conway&apos;s game of life is a classic zero player game that makes for
      ...
      <p>
        Maybe we can play around with alternative rule sets, or investigate
      ...
      <div>
        {map.map((row, rowIndex) => {
          return (
            <div key={`row-${rowIndex}`} className="game-of-life-row">
              {row.map((cell, cellIndex) => {
                return (
                  <div
                    className={`game-of-life-cell ${cell ? "live" : "dead"}`}
                    key={`cell-${cellIndex}`}
                    role="button"
                    onClick={() => {
