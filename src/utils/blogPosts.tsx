...
import type {
  EnrichedFrontMatterPlusSlug,
  FrontMatterPlusSlug,
} from "../../utils/frontmatterTypings";
 ...
 * @returns
 */
export async function getFrontmatterFromSlug(
  slug: string
): Promise<EnrichedFrontMatterPlusSlug> {
  ...
  if ("series" in data.frontmatter) {
    seriesFrontmatter = Object.values(allMetadata[folder])
      .filter((v) => {
...
export function mergeFrontmatterAndDefaultMetadata(
  meta: Partial<FrontMatterPlusSlug["frontmatter"]["meta"]> | null,
  path?: string
): Metadata {
  if (!meta) {
...
export async function getAllPostFrontmatter(
  group: BlogPostFolders = "posts"
): Promise<Array<FrontMatterPlusSlug>> {
  return Object.values(
    allMetadata[group] as Record<string, FrontMatterPlusSlug>
  ).sort((a, b) => {
    return (
      new Date(b.frontmatter.meta?.dateCreated ?? 0).valueOf() -
      new Date(a.frontmatter.meta?.dateCreated ?? 0).valueOf()
    );
  });
}
