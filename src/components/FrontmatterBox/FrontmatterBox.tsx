  ...
  return (
    <div className="series-box">
      <p>
        ...
        <i className="series-description-text">
          {firstSeriesItem.frontmatter.series?.description ??
            firstSeriesItem.frontmatter.series?.name}
        </i>
        &quot;{" "}
      </p>
      <ul>
        {props.frontmatter?.seriesFrontmatter?.map((v) => {
          return (
            <li key={v.slug}>
              <Link
                href={`/${v.slug}`}
                className={v.slug === props.frontmatter?.slug ? "current" : ""}
              >
    ...
    return (
      <div className="next-post">
        <Link href={`/${nextPost.slug}`}>
          <strong>Next:</strong> {nextPost.frontmatter.meta.title}
        </Link>
      </div>
    );
  }
...
export async function FrontmatterBox(
  ...
  return (
    <>
      ...
      <div>
        ...
        <DatePublished date={frontmatter.frontmatter.meta.dateCreated} />
      </div>
      {props.children}
      ...
      <>
        ...
        <a href="#comments">Get in the comments! 👇</a>
      </>
    </>
  );
}
