...
function FilterInformation(props: {
  ...
  return (
    <div>
      {props.filterInformation.tagFilter &&
        props.filterInformation.didFindArticles && (
          <h1>
            Articles tagged{" "}
            <i>&quot;{props.filterInformation.tagFilter}&quot;</i>:
          </h1>
        )}
      {props.filterInformation.tagFilter &&
        !props.filterInformation.didFindArticles && (
...
export function ListOfArticles(props: {
  ...
  return (
    <div data-testid="bsc-homepage-all-article-previews">
      {props.filterInformation && (
        <FilterInformation filterInformation={props.filterInformation} />
      )}
      ...
      {props.allFrontmatter.map((v) => {
        return (
          <Link
            ...
            data-testid="bsc-homepage-article-preview-box"
            aria-label={v.frontmatter.meta.title}
          >
            <div>
              <p className="homepage-article-link-title">
                {v.frontmatter.meta?.title ?? v.slug}
              </p>
              {v.frontmatter.meta?.dateCreated && (
                <DatePublished date={v.frontmatter.meta.dateCreated} />
              )}
              <p className="homepage-article-link-description">
