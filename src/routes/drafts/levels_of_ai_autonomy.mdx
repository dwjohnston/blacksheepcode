...
You prompt the agent and observe its reasoning response. You can intervene when the agent seems to be going down the wrong path. 
...
For me - this is where things really start breaking down. It's hard to visualise what the subagents are doing, it becomes hard to intervene.

It's worth mentioning that while it might be reasonable to want to visualise what subagents are doing when there are 2-5 agents, it's conceivable that some workflows might involve spawning hundreds of agents, at which point intervening live would never be a possibility, so any visualisation you had would be more catered around a 'after the fact' forensic accounting. 
