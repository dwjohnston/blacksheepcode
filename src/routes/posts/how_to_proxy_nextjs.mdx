...
I don't know. I am not currently aware of a mechanism to proxy the rewrites directly. 

I have a [discussion here](https://github.com/vercel/next.js/discussions/69558) requesting this functionality. 

Perhaps the way to go is to run a seperate process that acts as a simple proxy server in place of our API service, and because we have control of it, we proxy its traffic via mitmproxy. 

Even more simply, mitmproxy probably has a mechanism for making direct requests to it and forwarding them to the right place.
...
<GithubPermalinkRsc permalink="https://github.com/dwjohnston/nextjs-proxying/blob/85edce69f5dc0537dff05e9b54b308525196faa6/package.json#L7-L8"/>
