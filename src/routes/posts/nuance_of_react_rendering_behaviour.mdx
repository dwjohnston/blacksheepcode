...
In a [previous post](./no_react_context_is_not_causing_too_many_renders) I highlighted a highlighted the difference between two subtley different components - one that recieves its content via the a `children` prop, and the other that renders its content directly. 
