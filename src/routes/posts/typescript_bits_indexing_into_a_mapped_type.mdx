...
Here, we see that the `renderData` function is typed correctly, giving use the correct narrow type for the value accessed by that key. 

### ...but it doesn't really work

Look what happens when we try allow multiple columns be processed: 
...
Let's say we go rid of that generic parameter completely, and just did this: 
...
where the `value` parameter is `MyRowData["a"]` _only when the key is `"a"`. 
