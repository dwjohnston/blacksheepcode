...
The idea is, that developers should live with the codebase they've got, and focus on being effective within their current paradigm. But at some point, you allow yourself/the developers to do some kind of large scale change. The point is, it's a somewhat deliberate act, rather than something that is constantly grinding away in a potentially impulisve manner. 
...
The idea is, we should accept that our code is going to become a convulted mess. 

The trick then, is in allowing it to be a convulted mess that we can work with. 
...
This is where one idea I'm toying with, is that you should try avoid having the same development team be creating two services/modules that interact directly with each other. 
