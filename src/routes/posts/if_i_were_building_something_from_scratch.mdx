...
Particularly as it relates to testing, it's much easier to include some tests when you're writing something new, than it is to add tests later on. 
...
OpenAPI has a wealth of associated tooling, code generators, documentation generators and the like. If we take a 'spec first' strategy to building our application firstly we're going to 

### Storybook and Storybook Testing 

I don't see Storybook, or tests as 'extra work'. They're actually a _faster_ way of working as components can be created in isolation, without having to navigate to them with the application dev server.
...
We want a formalised release process that will 


### Observability

## Tooling I would add if we're hiring more people 
...
### Increase static checks 

At the time we're expanding the team, I would put greater focus on improving static checks.
...
For example, say we our product is some kind of credit card payment form that others will integrate in our website. 
