...
<p>Note that on a page refresh the values in the demo are blank and then they pop-in. This is to avoid hydration errors - this solution solves for cookies on the client-side only. The cookies are available to the server - a solution that avoids this flash will be the job for a seperate post. </p>
</InfoPanel>


The a polyfilled implementation of a responsive useCookie looks like this: 

<GithubPermalinkRsc permalink="https://github.com/dwjohnston/blacksheepcode/blob/6f33d3feb510f47b0b4f44e2add8c1e7171e26ae/src/demos/responsive_cookies/hooks/useCookieWithListenerPolyfilled.ts#L1-L136"/>
